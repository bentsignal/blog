...
export function Frame({
  ...
  if (isMobile) {
    return (
      <Sheet.Frame open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <Sheet.Content
    ...
    >
      ...
      >
        <div
          ...
          className="group-data-[variant=floating]:border-sidebar-border bg-sidebar/50 flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm"
        >
          {children}
        </div>
      </div>
...
export function Rail({ className, ...props }: React.ComponentProps<"button">) {
  const { toggleSidebar } = useSidebar();
...
export function Inset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main

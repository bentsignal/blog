...
description:
  "A deep dive into rethinking how we structure the code, files, and folders of
  our React projects"
...
<Callout
  type="important"
  className="mt-2"
  children="A lot has changed since I wrote this. I would not recommend strictly adhering to this pattern, especially if you're building with AI."
/>

If you've worked in a modern React project before, then you've probably seen
something like this:

<FolderView client:visible directory={folders.basicProject} />

Where files are sorted based on the type of code they contain. Hooks live with
other hooks, components with other components, etc.

For simple projects, this tends to work fine. But as your codebase scales, you
end up with _low feature cohesion_. Making changes to one area of your app
requires editing multiple files scattered far and wide across different
directories.

To see this in action, let's take a look at a project with a bit more
complexity: **a messaging app**.

<FolderView client:visible directory={folders.messagingAppFirstIteration} />

If you want to make changes to the way messages work in this app, you may need
to access components, hooks, contexts, types, and who knows what else. These
things all live in separate files & folders, with the only indication that they
are related being that:

1. they all _hopefully_ have message somewhere in their name
2. they _might_ import from each other.
...
<FolderView client:visible directory={folders.messagingAppSecondIteration} />
...
dedicated folder for components from shadcn registries[^1]. These are usually
...
<FolderView client:visible directory={folders.closerLookAtComponents} />

My issue with this convention is that it doesn't draw clear lines with regard to
where code should belong.

- Does having some of the component files in `📁/ui`mean that the others
  **aren't** UI?
- Since it's the components folder, that means that all of this is component
  code, right?
...
official [shadcn](https://ui.shadcn.com/) registry. As we can see, it defines
constants, a type, a context, and a hook before we get to any component code.
...
Is sidebar really a feature though? Looking over the
...
![Atomic Design Process](https://bsx-main.b-cdn.net/atomic-design.avif)
...
[^3]:
    ...
    those opinions as the first post. Here we are.
...
building in prescribes. For example, `layout.tsx` and `page.tsx` in
[Next](https://nextjs.org/docs/app/getting-started/layouts-and-pages) are fairly
analogous to templates and pages, so I think I'll just stick with those for now.
...
Let's go ahead and move all of these shadcn components from `📁/components/ui`
into a new folder: `📁/atoms`

<FolderView client:visible directory={folders.moveShadcnComponentsToAtoms} />
...
<FolderView client:visible directory={folders.sidebarAtomSplitUp} />
...
Great! We've given all of our shadcn[^4] components a new home. What about the
components we wrote ourselves? What should we do with those?
...
<FolderView client:visible directory={folders.closerLookAtMessagesFeature} />
...
[^5]:
    ...
    the [repo](https://github.com/bentsignal/blog) for this project.
...
When combining the power our new message atom with more general purpose atoms
like _list_ and _scroll_, it becomes simple to compose something like the
message list molecule shown below.
...
<FolderView
  client:visible
  directory={folders.messagesFeatureAfterAtomicDesign}
/>

That's it! Putting atoms and molecules into feature folders is the last piece of
the puzzle. Let's see how things look after applying our new Atomic Design
principles to every corner of the app:

<FolderView client:visible directory={folders.messagingAppThirdIteration} />
...
We have dramatically improved the readability, maintainability, and
composability of our project. Adopting this practice[^6] will put you on a path
towards consistent UI and improved code clarity.
...
Enough from me, I want to know what you think. Would you consider adopting this

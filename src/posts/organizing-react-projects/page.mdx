...
very common to have a folder thast contains prebuilt components from a
...
`📁/features`. Which brings us to our next topic: **_Atomic Design_**.
...
Design, while adjusting the definitions to better fit the needs of the modern
React project.
...
perfectly aligned with their real relationship in chemistry, but its close
enough that the analogy is still useful.
...
project. That way you can still think of all of the sidebar code as living
together, even though it is spread across different files.
...
// ✅ as long as your bundler can handle it
...
This is great. When we have an atom like messages and molecules that are tied
closely to it, we can store them all in a feature folder. For more vague atoms
like the previously mentioned _list_ and _scroll_, we can just store them in
`📁/atoms`
...
We can dramatically improve the readability, maintainability, composability —
basically all the \*abilities of our code.

I'm not claiming this system is perfect for any and all codebases. I've never
encountered the challeneges that lie from codebases with hundreds of thousands
of lines of code.

But for the 10,000 or so lines of code it took to create this blog site with the
messaging app in the sidebar, I found this practice incredibly plesant to use.

But now I want to know what YOU think about it. Leave me a message on the left,
and lets chat about it. You can also find me on pretty much any social media,

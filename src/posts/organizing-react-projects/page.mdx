...
If you've worked inside a modern React project before, then you've probably seen
something like this:

<FolderView directory={folders.basicProject} />

Where files are split up depending on the type of code they contain. Hooks live
...
that feature. For example, all the code for messages will now live in
`📁/features/messages`
...
This feels much better. Now when you want to make changes to messages, you only
have to look in one place: `📁/features/messages`
...
very common to have a folder that contains prebuilt components from a
...
where. Does having some of the component files in `📁/ui` mean that the others
...
its more like a collection of building blocks that we can put together to build
a sidebar tailored to the _specific needs of our app_.
...
perfectly aligned with their relationship in chemistry, but its close enough
...
- ✅ **_Can_** be built with other atoms
- ❌ **_Cannot_** be built with molecules
...
- ✅ **_Can_** be built with atoms
- ✅ **_Can_** be built with other molecules
...
Lets go ahead and move all of these shadcn components from `📁/components/ui`
into a new folder: `📁/atoms`
...
hundreds of lines of code. If you'd like to split it up, feel free to do so by
creating a sidebar folder and separating the code by type.
...
project. That way you can still think of all of the sidebar code as being
encapsulated in one object, even though it is spread across different files.
...
// ✅ most bundlers can handle this without increasing bundle size
...
Lets take another look at the feature folder we created earlier for our messages
code:
...
their code that serve simliar or possibly identical purposes. If we split that
code out into reusable pieces, then it could be shared among our different types
of messages.

Bells should be ringing in your head right now. Lets create a message atom[^5],
containing small reusable components that our message variants share. Then we
can compose our three message variants as molecules, built off of the message
atom.

[^5]:
    I won't include code snippets for any of the atoms I've built since they're
    all fairly long. If you're interesting in reading through them, check out
    the [repo](https://github.com/bentsignal/blog) for this site.
...
This practice[^6] will give us an easier path to consistency within our UI, make
it easier to build new message variants down the road, and make it much easier
for both humans and AI to understand and modify our code.

[^6]:
    This is all part of a larger pattern: building your components like shadcn.
    To better understand the power of building like this, I highly recommend you
    watch
...
I also built _list_ & _scroll_ atoms. Each of these contains many components
that aren't terribly useful on their own. But when put together, it becomes
trivial to compose something like the message list molecule shown below.
...
Lets take another look at our messages feature folder now that we've reorganized
things:

<FolderView directory={folders.messagesFeatureAfterAtomicDesign} />

This is great. When we have an atom and molecules that are tied closely to it,
we can store them all in a feature folder. General purpose atoms like the
previously mentioned _list_ and _scroll_, can just be stored in `📁/atoms`

Thats the last piece of the puzzle, lets and see how things look after applying
our new Atomic Design principles to every corner of the app:

<FolderView directory={folders.messagingAppThirdIteration} />

By following these principles:

1. Grouping code by **feature**
2. Building **atoms**, and then composing **molecules** from those atoms.

We have dramatically improved the readability, maintainability, composability —
basically all the \*abilities of our code.

I'm not claiming this system is perfect for any and all projects. I've never
encountered the challeneges that come from codebases with hundreds of thousands
of lines of code.

But for the 10,000 or so lines of code it took to create this blog site with a
messaging app in the sidebar, I found this practice incredibly pleasant to use.

But now I want to know what YOU think about it. Would you consider adopting this
pattern into your projects? If not, what issues do you see with it? What would
you do differently?

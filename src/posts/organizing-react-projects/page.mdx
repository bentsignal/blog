import { FolderView } from "@/features/directory/molecules";
...
Lets create a message atom[^5], containing small reusable components that our
...
Adopting this practice[^6] throughout our entire codebase will put us on a path
towards consistency within our UI, simplify the process of creating new message
variants down the road, and improve code clarity for both humans and AI.
...
I also built two atoms named list and scroll. Each of these contains many
components that aren't terribly useful on their own. But when put together, it
becomes trivial to compose something like the message list molecule shown below.
...
Lets take another look at our messages feature folder now that we've
restructured our code:

<FolderView directory={folders.messagesFeatureAfterAtomicDesign} />

Great. Now when we have molecules that are tied closely to one specific atom, we
can store them all in a feature folder. Meanwhile, general purpose atoms like
list and scroll can stay in `📁/atoms`
...
This system may not be the best fit for all projects. I've never encountered the
challeneges that come from codebases with hundreds of thousands of lines of
code. But for the 10,000 or so lines of code it took to create this blog site
with a messaging app in the sidebar, I found this practice incredibly pleasant
to use.

You can also adopt this pattern incrementally, no need to overhaul your entire
project in one go.

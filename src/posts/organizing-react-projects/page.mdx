...
To see this in action, lets take a look at a more complicated project: **a
messaging app**.

<FolderView directory={folders.messagingAppFirstIteration} />

Things are starting to get out of hand. `📁/components` is growing much faster
than the other directories, which makes sense. In React, everything you can see
on your screen is a component, so this folder is destined to grow quickly.
...
may need to access the message components, the message actions hook, the message
...
files in a new folder: `📁/features`
...
code is in one place: `📁/features/message`
...
very common to have a folder named `📁/components/ui` which contains prebuilt
...
our component files in `📁/ui` mean that the others aren't UI?
...
Should we then put them in a new folder in `📁/features`?

I don't think we should.

I think moving code into separate files can be helpful if we expect to work with
it regularly down the road. For example: as our app grows and we add new
features, there's a good chance we'll be making changes and refactoring messages
along the way. I think separating out the code makes this easier, as you don't
have to swim through one monolithic file to find what you're looking for.

But for something like the sidebar, I think its actually quite nice to have all
of the code colocated in a single file. While we may need to make some changes
here and there to adapt it to our project, chances are it won't be very often.
...
most of us are already building in React, but with explicit guidelines for how
to order components.
...
was drawing the lines between atoms, molecules, and organisms.

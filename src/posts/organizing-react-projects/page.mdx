...
<FolderView directory={messagingAppFirstIteration} />

As you can see, things are starting to get out of hand. `📁/components` is
...
<FolderView directory={messagingAppSecondIteration} />
...
Where `📁/components/ui` contains prebuilt components from a library like
...
where. Does having some of the files in `📁/ui` mean that the others aren't UI?
Is the code in these files solely React components? We're in `📁/components`, so
...
We could create `📁/features/sidebar` and put the previously mentioned files in
